// Controller: parsea/valida filtros y delega en el model
...
const obtenerProductosDetallado = async (req, res) => {
  try {
    ...
    // Paginación/orden válidos (whitelist para evitar inyección en ORDER BY)
    ...
    const direccionOrden = direccionesPermitidas.includes(String(order).toLowerCase())
      ? String(order).toLowerCase()
      : 'desc';

    // Normalizar canales CSV (case-insensitive). Acepta "vitex,meli" o &canal=vitex&canal=meli
    const CANAL_MAP = {
      meli: 'Meli',
      falabella: 'Falabella',
      balmaceda: 'Balmaceda',
    ...
    // Normalizar SKUs CSV (split por coma; los puntos son parte del SKU)
    ...
    // ====== CALL MODEL ======
    const result = await obtenerProductosDetalladoDB({
      canalesCsv,
      skuCsv,
