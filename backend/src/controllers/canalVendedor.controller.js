    ...
    const query = `
      ...
      ;WITH VentasVendedores AS (
          SELECT 
              ...
              S.U_Imagen AS Imagen,
              SUM(I.Quantity) AS CantidadVendida,
              SUM(I.LineTotal) AS TotalVentas,
              SUM(I.LineTotal - I.StockPrice * I.Quantity) AS MargenBruto,
              CAST(
                  CASE 
                      WHEN SUM(I.LineTotal) = 0 THEN 0
                      ELSE (SUM(I.LineTotal - I.StockPrice * I.Quantity) * 100.0) / SUM(I.LineTotal)
                  END AS DECIMAL(5,2)
              ) AS MargenPorcentaje
          FROM INV1 I
          ...
          WHERE 
              ...
              AND (
                  @CanalParam IS NULL
                  OR (
                      (@CanalParam = 'Meli' AND ((I.WhsCode IN ('03', '05') AND T0.SlpCode IN (426, 364, 355, 398))
      ...
      TotalCantidad AS (
          SELECT SUM(CantidadVendida) AS Total FROM VentasVendedores
      )
      SELECT top 10
          V.Codigo_Vendedor,
          V.Nombre_Vendedor,
          ...
          V.MargenBruto,
          V.MargenPorcentaje,
          ROUND((V.CantidadVendida / NULLIF(T.Total, 0)) * 100, 2) AS Porcentaje
      FROM VentasVendedores V
      CROSS JOIN TotalCantidad T
