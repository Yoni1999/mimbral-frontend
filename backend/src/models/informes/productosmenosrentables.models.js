...
async function obtenerProductosDetallado(params = {}) {
  const {
    canal = null,          // string: "Meli" o "Meli,Chorrillo,Empresas"
    canales = null,        // compat, por si llegara array (no necesario si ya usas solo 'canal')
    vendedor = null,
  ...
  // Columnas ordenables EXACTAS del SELECT
  const orderMap = {
  ...
  // Soporte 1 o N canales: construimos CSV si viene array (compat) o usamos 'canal'
  const canalesCsv =
    Array.isArray(canales) && canales.length
      ? canales.join(',')
      : (canal ? String(canal) : null);
...
CROSS APPLY (VALUES (
  CASE 
    WHEN ((I.WhsCode IN ('03','05') AND T0.SlpCode IN (426,364,355))
          OR (I.WhsCode = '01' AND T0.SlpCode IN (355,398))) THEN 'Meli'
    ...
    -- 👇 PRIORIDAD: EMPRESAS basado en SlpCode de la LÍNEA (INV1)
    WHEN (I.WhsCode = '01' AND I.SlpCode IN (227,250,205,138,209,228,226,137,212)) THEN 'Empresas'
    -- 👇 CHORRILLO = complemento, también evaluado con SlpCode de la LÍNEA
    WHEN (I.WhsCode = '01' AND (I.SlpCode IS NULL OR I.SlpCode NOT IN (227,250,205,138,209,228,226,137,212))) THEN 'Chorrillo'
    ELSE NULL
  END
)) AS C(CanalCalc)
WHERE
  ...
  AND T0.CANCELED = 'N'
  AND I.Quantity > 0
  AND T1.WhsCode = '01'               -- bodega filtro
  ...
  -- Canales múltiples (CSV)
  AND (
    @CanalesCsv IS NULL
    OR EXISTS (
      SELECT 1
      FROM STRING_SPLIT(@CanalesCsv, ',') s
      WHERE LTRIM(RTRIM(s.value)) = C.CanalCalc
    )
  )

  -- Vendedor por cabecera o línea
  AND (@VendedorParam IS NULL OR T0.SlpCode = @VendedorParam OR I.SlpCode = @VendedorParam)
...
`;
  ...
  // Bind
  request.input('CanalesCsv',        sql.VarChar(200), canalesCsv); // CSV de 'canal'
